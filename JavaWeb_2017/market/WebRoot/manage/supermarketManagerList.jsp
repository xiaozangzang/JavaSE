...
<% 
	...
	String baseManagerUrl=basePath+"supermarketManager";
	//列表数据
	List<SupermarketManager> list = (List<SupermarketManager>)request.getAttribute("list");
    long supermarketId=(Long)request.getAttribute("supermarketId");
    request.setAttribute("supermarketId",supermarketId);
%>
...
<div class="manage_right">
	<div class="content">
		...
		<div class="manageButton">
			<a class="button" href="<%=basePath %>/manage/<%=type %>Add.jsp?supermarketId=<%=supermarketId %>">新增</a>
		</div>
...
$(function(){
	...
	$('#prev').click(function(e){
		if(startNum != 0){
			location.href = '<%=baseUrl%>/queryList?id=<%=supermarketId%>&startNum=' + (startNum -pageCount)
		}
	});
