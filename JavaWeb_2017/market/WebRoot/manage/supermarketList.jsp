...
<% 
	...
	//列表数据
	List<Supermarket> list = (List<Supermarket>)request.getAttribute("list");
%>
				...
				<tbody>
					...
					<tr>
						...
						<td>
							...
							<a data-id="<%=list.get(i).getId() %>" href="#" class="manager">
								<span>超市管理员</span>
							</a>
						</td>
					</tr>	
...
$(function(){
	...
	$('.manager').click(function(e){
		var id = $(this).data('id');
		if(id){
			location.href = '<%=baseManagerUrl%>/queryList?id='+ id + '&startNum=<%=startNum%>&pageCount=<%=pageCount%>'
		}
	})
})
</script>
